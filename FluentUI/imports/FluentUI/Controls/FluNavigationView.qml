...
Item {
    ...
    // 2026-08-26 16:20 v9: ToolHub navMode 配置守卫 (库不能直接读 ToolHub.Settings,
    // 业务层 Settings.applyNavMode / setNavMode 同步设这个 property)
    // false (默认) = 走 Example 库默认 popup 行为 (业务态 0/1 锁死)
    // true       = 走 v3 决策 toggle 行为 (业务态 2 自动模式, expander 父项 Compact→Open / Open→Compact)
    property bool _navAutoMode: false
            ...
            FluControl{
                ...
                onClicked: {
                    if(d.isCompactAndNotPanel && model.children.length > 0){
                        // 2026-08-26 16:20 v9 守卫: 业务态 2 (自动) 走 v3 决策 toggle
                        // 业务态 0/1 走 Example 库默认 popup (锁死)
                        // 决策: decisions/2026-08-26_upstream-pr-restoration.md
                        if(control._navAutoMode){
                            // 业务态 2 自动: Compact 模式点 expander 父项 → 展开 sidebar + 展开子项 + 选中
                            // (跟搜索栏自动展开行为一致, 跟 v3 决策 ui-nav-click-behavior 一致)
                            control.displayMode = FluNavigationViewType.Open
                            model.isExpand = true
                            nav_list.currentIndex = _idx
                            return
                        }
                        // 业务态 0/1 (默认 Example 库 popup 行为)
                    ...
                    // Open/Minimal 模式
                    // 业务态 2 重复点同 expander → 折叠 sidebar (v3 决策 toggle 闭环)
                    if(control._navAutoMode && nav_list.currentIndex === _idx){
                        control.displayMode = FluNavigationViewType.Compact
                        return
                    }
                    // 库默认: 展开/折叠子项
                    model.isExpand = !model.isExpand
                    // 业务态 2: 父项自身被点时也算"选中"（蓝条显示，跟 v3 一致)
                    if(control._navAutoMode){
                        nav_list.currentIndex = _idx
                    }
                }
