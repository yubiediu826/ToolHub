...
a = Analysis(
    ...
    datas=[
        ('./source.zip', '.'),
        # FluentUI/imports/ 是 namespace package（无 __init__.py）。
        # 显式添加 resource_rc.py（QRC 编译产物，必须在 FluentUI/imports/ 下）
        # 和 QML 资源目录（Controls/Font/Image/i18n/JS），让 Qt 运行时能加载。
        (os.path.abspath('./FluentUI/imports/resource_rc.py'), 'FluentUI/imports'),
        (os.path.abspath('./FluentUI/imports/FluentUI'), 'FluentUI/imports/FluentUI'),
    ],
    hiddenimports=[
        # 让 PyInstaller 把 FluentUI.imports 当作包处理
        'FluentUI.imports',
        'FluentUI.imports.resource_rc',
    ],
    hookspath=[],
    hooksconfig={},
    runtime_hooks=[],
